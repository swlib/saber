...
| keep_alive            | bool                  |                                | `true` \| `false`                                            | The default is true, the connection will be reused automatically when redirecting (currently, you need set `use_pool` to `true` if you want enable `keep_alive` to all possible connections,not only automatically redirecting  ） |
